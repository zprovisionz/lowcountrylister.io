...
export default function ResultsDisplay({
  ...
  const tabs: Array<{
    ...
    { id: 'mls' as const, label: 'MLS/Zillow Description', show: true },
    { id: 'airbnb' as const, label: 'Airbnb/VRBO Description', show: !!airbnbDescription },
    { id: 'social' as const, label: 'Social Media Captions', show: !!socialCaptions },
    { id: 'staging' as const, label: 'Virtual Staging', show: true, badge: stagedImages.length > 0 ? stagedImages.length : undefined },
  ].filter((tab) => tab.show);

  return (
    <div className="glass rounded-2xl shadow-2xl p-6 md:p-8 space-y-6 animate-fadeInUp">
      <div className="space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div
            className={`flex items-center gap-2 px-4 py-2 rounded-lg animate-scaleIn ${
        ...
        {activeTab === 'mls' && (
          <div className="space-y-4">
            <div className="bg-gray-900/50 rounded-xl p-6 border border-gray-700/50 relative">
              {isAnonymous && previewSnippet ? (
                <>
                  <div className="relative">
                    <p className="text-gray-200 leading-relaxed whitespace-pre-wrap">
                      {previewSnippet}
                    </p>
                    <div className="absolute inset-0 bg-gradient-to-b from-transparent via-gray-900/80 to-gray-900 pointer-events-none"></div>
                    <div className="absolute bottom-0 left-0 right-0 p-4 bg-gray-900/95 backdrop-blur-sm border-t border-gray-700/50">
                      <div className="flex items-center justify-between gap-4">
                        <p className="text-sm text-gray-300">
                          {remainingGenerations !== undefined && remainingGenerations > 0
                            ...
                            : 'Sign up to view full description'}
                        </p>
                        <button
                          onClick={onUpgradeClick}
                          className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium text-sm"
                        >
                          Sign Up for Full Description
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              {subscriptionTier === 'free' && !isAnonymous && (
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 disabled:opacity-50 disabled:cursor-not-allowed btn-ripple ${
                  copiedIndex === 'mls' 
                    ? 'bg-green-600 text-white' 
                    : 'bg-blue-600 text-white hover:bg-blue-700 btn-glow'
                }`}
              ...
              >
                {copiedIndex === 'mls' ? (
                  <>
                    <Check className="w-4 h-4 animate-success" aria-hidden="true" />
                    Copied!
                  </>
                ) : (
                  <>
